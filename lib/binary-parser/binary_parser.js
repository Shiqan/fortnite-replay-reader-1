...
Parser.prototype.if = function (varName, options) {
    ...
    if (!options.check) {
        throw new Error("Tag option of if is not defined.");
    }

    if (options.check) {
        return this.setNextParser("if", varName, options);
    }
};
...
Parser.prototype.generateIf = function (ctx) {
    if (this.varName) {
        ctx.pushCode("{0} = {};", ctx.generateVariable(this.varName));
    }
    ctx.pushCode("if ({0}) {", this.options.check);
    var type = this.options.parser;
